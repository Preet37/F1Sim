...
export function resolvePitStop(req: PitStopRequest, rng: Rng): PitStopResult {
  ...
  for (const corner of WHEEL_CORNERS) {
    ...
    corners.push({ corner, doneS: JACKS_UP_S + t, problem });
...
export function pitStopProgress(
  ...
  const up = span(elapsedS, 0, JACKS_UP_S);
  const down = span(elapsedS, dropStart, dropEnd);
  into.jack = up * (1 - down);

  for (let i = 0; i < r.corners.length; i++) {
    const c = r.corners[i];
    const work = c.doneS - JACKS_UP_S;
    const t = work <= 0 ? 1 : span(elapsedS, JACKS_UP_S, c.doneS);
  ...
  into.nose = r.noseChange
    ? span(elapsedS, JACKS_UP_S, JACKS_UP_S + Math.max(r.extraWorkS, 0.5))
    : 0;
