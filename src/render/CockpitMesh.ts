...
/** Steering wheel: a squared-off rim with grips, angled back toward the driver. */
function steeringWheel(): THREE.BufferGeometry[] {
  const parts: THREE.BufferGeometry[] = [];
  const tilt = -0.62;
  ...
  for (const side of [-1, 1] as const) {
    ...
    grip.rotateX(tilt);
    grip.translate(side * 0.105, WHEEL_Y - 0.005, WHEEL_Z + 0.004);
...
function figure(segments: number): THREE.BufferGeometry[] {
  ...
  for (const side of [-1, 1] as const) {
    ...
    const hx = side * 0.108, hy = WHEEL_Y - 0.008, hz = WHEEL_Z - 0.035;
    ...
    glove.translate(hx, hy, hz);
    parts.push(tag(glove, 'glove'));
  }

  parts.push(...steeringWheel());
  return parts;
}
