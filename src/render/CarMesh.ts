...
import { buildDriverParts } from './CockpitMesh';
 ...
 *  - a DRIVER, with a helmet and shoulders and arms. See CockpitMesh.ts.
 *
...
export interface CarVisual {
  ...
  /**
   * The driver's helmet. Hidden for the player's own cockpit camera, which sits
   * inside it — otherwise that view renders the inside of a helmet shell.
   */
...
function buildShellParts(quality: 'low' | 'high'): THREE.BufferGeometry[] {
  ...
  for (const side of [-1, 1] as const) {
    p.flat(strut(side * 0.300, 0.560, 0.545, side * 0.455, 0.620, 0.548, 0.014, 5), 'trim');
    const housing = new THREE.BoxGeometry(0.105, 0.058, 0.040);
    housing.translate(side * 0.500, 0.624, 0.546);
    p.flat(housing, 'body');
    const glass = new THREE.PlaneGeometry(0.088, 0.046);
    glass.rotateY(Math.PI);
    glass.translate(side * 0.500, 0.624, 0.525);
    p.flat(glass, 'glass');
  }
  ...
  // --- Driver and cockpit -------------------------------------------------
  p.list.push(...buildDriverParts(quality).body);
...
function geometryFor(quality: 'low' | 'high'): CachedGeometry {
  ...
  const built: CachedGeometry = {
    shell: mergeParts(buildShellParts(quality)),
    head: mergeParts(buildDriverParts(quality).head),
