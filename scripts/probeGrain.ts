 ...
 *
 * THE ASSERTION is `near / mid` on the road: band 5 over band 2. The bound is
 * not a taste judgement. Every band of the surface detail is faded out on its
 * own screen-space footprint by `detailResolve` in `SurfaceDetail.ts` precisely
 * so that no band is ever drawn at a frequency the pixel grid cannot carry;
 * if that holds, high-frequency energy per pixel must not RISE as the asphalt
 * comes closer, because the near field is where the footprint is smallest and
 * every band is therefore inside its limit. A correctly band-limited road
 * measures a ratio at or below 1. The bound allows for the genuine fact that
 * near asphalt resolves real aggregate that distant asphalt cannot.
 *
 * Usage:
 *   npm run probe:grain
...
async function main(): Promise<void> {
        ...
        for (const mode of MODES) {
          ...
          if (m.error || !m.road) {
            rows.push({
              ...
              nearOverMid: null, ok: false, note: m.error ?? 'no measurement',
            });
          ...
          const ok = ratio === null ? true : ratio <= NEAR_OVER_MID_MAX;
          const note = ratio === null
            ? `too little asphalt in band 2 or 5 (${counts[2] ?? 0}/${counts[5] ?? 0}px)`
            : ok ? '' : `near-field asphalt is ${ratio.toFixed(2)}x the mid-distance`
  ...
  await writeFile(resolve(OUT_DIR, 'manifest.json'),
    JSON.stringify({ bound: NEAR_OVER_MID_MAX, rows }, null, 2), 'utf8');
  await browser.close();
  await server.close();

  const failed = rows.filter((r) => !r.ok);
  console.log('\n=== ROAD GRAIN BY DEPTH — issue #48 ===');
  console.log(`bound: near-field asphalt <= ${NEAR_OVER_MID_MAX.toFixed(2)}x`
    + ' the mid-distance (mean |Laplacian| of luma, road pixels only)');
